...
from src.tac.training.tacotron_training import get_infolog_path, get_save_dir
...
def model_train_mode(args, feeder, hparams, global_step, init=False):
  with tf.variable_scope('WaveNet_model', reuse=tf.AUTO_REUSE) as scope:
    model = create_model(hparams, init)
...
def get_save_dir(log_dir):
  return os.path.join(log_dir, 'wave_pretrained')
